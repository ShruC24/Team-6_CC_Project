...
def get_data_brokers():
    if node.state != 'l':
        return jsonify({'error': 'Only the leader node can perform create operation'}), 403
...
def update_task(id):
    if node.state != 'l':
        return jsonify({'error': 'Only the leader node can perform update operation'}), 403
    data = request.json
    title = data.get('TITLE')
    task_status = data.get('TASK_STATUS')
    created_by = data.get('CREATED_BY')
